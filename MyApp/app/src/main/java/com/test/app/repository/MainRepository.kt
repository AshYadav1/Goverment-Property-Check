...
class MainRepository(
    ...
    private val retrofitServiceForFirebase: RetrofitService?,
    private val restApiForMoveItem: RetrofitService? = null
) {
    ...
    fun readQrCode(qrCode: String) = retrofitService.readQrCode(
        ...
        ).getString("org_slug", ""), qrCode
    )


    fun readUserDetailByQr(qrCode: String) = retrofitService.readUserQr(qrCode)

    fun getLicenseId(qrCode: String) = retrofitService.getLicenseId(qrCode)
    fun patchData(qrCode: String, model: RequestModelHolder) =
        retrofitServiceForFirebase?.patchFirebase(qrCode, model)
